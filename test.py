...
def shortest_path(source, target):
    ...
    Args:
        Source = Name of first actor
        Target = Name of second actor
    ...
    """
    source_id = str(person_id_for_name(source))
#    print(f"source_id is {source_id}")
    target_id = str(person_id_for_name(target))
#    print(f"target_id is {target_id}")

    #initialise start node
    start = Node(state = source_id,    #node.state = actor ID
    ...
    while True:
        ...
        #print(f"Nodes Explored = {numexplored}. Current node = {node.state}")


        #If node is the target, return the route taken to get there
        if node.state == target_id:
            #print(f"Triggered node.state == target_id. node.state = {node.state}")
        ...
        for movie_id, costar in neighbors_for_person(node.state):
            if not frontier.contains_state(costar) and costar not in actorsexplored:
                child = Node(state = costar,
                             ...
                             action = movie_id)
                frontier.add(child)
                #print(f"Child: {child.state} created and added to frontier.")
